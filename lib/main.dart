...
// ── Vault tile display (mirrors the Home tab: unified indigo) ──────
...
class _VaultPageState extends State<VaultPage> {
  ...
  @override
  Widget build(BuildContext context) {
    // A tile was tapped -> show the filtered list (all existing code).
    if (_selectedCategory != null) {
      return _buildListView();
    }
    // Default: shared tile grid (mirrors Home tab).
    return Scaffold(
    ...
    return LayoutBuilder(builder: (ctx, constraints) {
      ...
      for (int i = 0; i < _tiles.length; i++) {
        ...
        children.add(AnimatedPositioned(
          ...
          left: col * (cellW + gap), top: row * (cellH + gap),
          width: cellW, height: cellH,
          child: _VaultTileCard(
            key: ValueKey(tile.id), tile: tile, width: cellW, height: cellH,
            organizeMode: _organizeMode, removing: _removingIds.contains(tile.id),
            index: i,
  ...
  /// Filtered list view shown after tapping a tile. Reuses all
...
// ── Vault list card (item in filtered list) ──────────────────────
      ...
      child: AnimatedScale(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [BoxShadow(color: const Color(0xFF5B3FE8).withOpacity(0.06), blurRadius: 16, offset: const Offset(0, 4))],
          ),
          child: Row(children: [
            Container(
              width: 52, height: 52,
              decoration: BoxDecoration(color: item.iconBgColor, borderRadius: BorderRadius.circular(14)),
              child: Icon(item.icon, size: 26, color: item.iconColor),
            ),
            const SizedBox(width: 14),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
              Text(item.title, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: Color(0xFF12101E), height: 1.2), maxLines: 1, overflow: TextOverflow.ellipsis),
              const SizedBox(height: 4),
              Text(item.subtitle, style: const TextStyle(fontSize: 13, color: Color(0xFF8E8BA8), height: 1.3), maxLines: 1, overflow: TextOverflow.ellipsis),
            ])),
          ]),
        ),
      ),
...
// ── Vault tile (unified indigo, drag / delete / tap) ─────────────
  ...
  Widget build(BuildContext context) {
    ...
    final card = AnimatedScale(
      ...
      child: AnimatedOpacity(
        opacity: widget.removing ? 0.0 : (_entered ? 1.0 : 0.0), duration: const Duration(milliseconds: 200),
        child: AnimatedBuilder(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [BoxShadow(color: _cPrimary.withOpacity(0.06), blurRadius: 14, offset: const Offset(0, 4))],
            ),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Container(width: 48, height: 48, decoration: BoxDecoration(color: _cPrimary.withOpacity(0.12), borderRadius: BorderRadius.circular(16)), child: Icon(info.icon, color: _cPrimary, size: 24)),
    ...
    if (!widget.organizeMode) {
      return GestureDetector(onTapDown: (_) => setState(() => _pressed = true), onTapUp: (_) => setState(() => _pressed = false), onTapCancel: () => setState(() => _pressed = false), onTap: widget.onTap, onLongPress: widget.onLongPress, child: card);
    }
    ...
    return DragTarget<int>(
      ...
      builder: (ctx, cand, rej) => LongPressDraggable<int>(
        data: widget.index,
        feedback: Material(color: Colors.transparent, child: Transform.scale(scale: 1.06, child: SizedBox(width: widget.width, height: widget.height, child: card))),
...
class _VaultTopBar extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 16), decoration: const BoxDecoration(color: Color(0xCCF4F3FF)), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
      ...
      AnimatedSwitcher(duration: const Duration(milliseconds: 200), child: organizeMode ? _VaultPill(key: const ValueKey('d'), label: 'Done', onTap: onDone) : const SizedBox(key: ValueKey('e'))),
    ]));
  }
}
class _VaultPill extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Material(color: _cPrimary, borderRadius: BorderRadius.circular(9999), child: InkWell(onTap: onTap, borderRadius: BorderRadius.circular(9999), child: Padding(padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 8), child: Text(label, style: _font(14, FontWeight.w500, Colors.white)))));
}
class _VaultFab extends StatelessWidget {
  final VoidCallback onTap; const _VaultFab({required this.onTap});
  @override
...
class _VaultEmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(child: Padding(padding: const EdgeInsets.all(32), child: Column(mainAxisSize: MainAxisSize.min, children: [
    Container(width: 72, height: 72, decoration: BoxDecoration(color: _cPrimary.withOpacity(0.12), shape: BoxShape.circle), child: const Icon(Icons.dashboard_customize, size: 32, color: _cPrimary)),
    const SizedBox(height: 20),
    Text('No shortcuts yet', style: _font(18, FontWeight.w600, _cOnSurface)),
    const SizedBox(height: 8),
    Text('Tap the + button to add your first one', style: _font(14, _cOnSurface.withOpacity(0.6)), textAlign: TextAlign.center),
  ])));
}
class _RemoveBadge extends StatelessWidget {
  final VoidCallback onTap; const _RemoveBadge({required this.onTap});
  @override
...
class _VaultAddSheet extends StatelessWidget {
  final List<HomeTileType> available; const _VaultAddSheet({required this.available});
  @override
  Widget build(BuildContext context) => SafeArea(child: Container(margin: const EdgeInsets.all(12), padding: const EdgeInsets.fromLTRB(20, 20, 20, 24), decoration: BoxDecoration(color: _cSurface, borderRadius: BorderRadius.circular(28)), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
    Center(child: Container(width: 40, height: 4, margin: const EdgeInsets.only(bottom: 16), decoration: BoxDecoration(color: _cPrimary.withOpacity(0.2), borderRadius: BorderRadius.circular(4)))),
    ...
    const SizedBox(height: 16),
    if (available.isEmpty) Padding(padding: const EdgeInsets.symmetric(vertical: 24), child: Text('All shortcuts are already on your Vault.', style: _font(14, FontWeight.w500, _cOnSurface.withOpacity(0.6))))
    else ...available.map((t) {
      final info = _kVaultTileInfo[t]!;
      return InkWell(borderRadius: BorderRadius.circular(16), onTap: () => Navigator.of(context).pop(t), child: Padding(padding: const EdgeInsets.symmetric(vertical: 10), child: Row(children: [
        Container(width: 42, height: 42, decoration: BoxDecoration(color: _cPrimary.withOpacity(0.12), borderRadius: BorderRadius.circular(12)), child: Icon(info.icon, color: _cPrimary, size: 20)),
        const SizedBox(width: 14),
