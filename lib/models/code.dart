...
// Local JSON serialization for GhostKey's local-first store. Replaces
// ente's `package:freezed` codegen to keep the demo dependency-light.
extension CodeJson on Code {
...
extension CodeFromMap on Code {
  static Code fromMap(Map<String, dynamic> map) {
    final algoName = (map['algorithm'] as String?)?.toLowerCase() ?? 'sha1';
    final algorithm = Algorithm.values.firstWhere(
      (a) => a.name == algoName,
      orElse: () => Algorithm.sha1,
    );
    final typeName = (map['type'] as String?)?.toLowerCase() ?? 'totp';
    ...
    return Code(
      ...
      (map['digits'] as int?) ?? Code.defaultDigits,
      (map['period'] as int?) ?? Code.defaultPeriod,
      map['secret'] as String? ?? '',
      algorithm,
      type,
      (map['counter'] as int?) ?? 0,
      map['rawData'] as String? ?? '',
      display: CodeDisplay.fromJson(
        (map['display'] as Map<String, dynamic>?) ?? const {},
      ),
    );
  }
}
